...
namespace internals{
    ...
    template<>
    struct type_to_value<std::false_type>{
        static const bool value = false;
    };
    ...
    struct conjunction<T1,T...>{
        static const bool value = type_to_value<std::conditional<T1::value,conjunction<T...>,std::false_type>>::value;
    };
}


template<typename T>
using is_zero = internals::conjunction<
...
template<typename T>
using is_no_copy_move = internals::conjunction<
    internals::negation<std::is_copy_constructible<T>>,
    internals::negation<std::is_copy_assignable<T>>,
    internals::negation<std::is_move_constructible<T>>,
    internals::negation<std::is_move_assignable<T>>
