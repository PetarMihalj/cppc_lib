...
template<typename T>
cppc::unique_lock<T>::~unique_lock(){
    this->_mutex->unlock();
}

template<typename T>
cppc::unique_lock<T>::unique_lock(cppc::unique_lock<T>&& that):_mutex(that._mutex){};

template<typename T>
cppc::unique_lock<T>& cppc::unique_lock<T>::operator=(cppc::unique_lock<T>&& that){
